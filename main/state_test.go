...
func TestStateString(t *testing.T) {
	...
	if "{lastPlayer:0, nbCreatures:0, creatures:map[], player:, foe:, round:0}" != text1 {
	...
	text2 := NewState().
		...
		String()
	if "{lastPlayer:2, nbCreatures:2, creatures:map[1:{id:1, color:1, type:1, coords:, scannedBy:[]} 2:{id:2, color:2, type:2, coords:, scannedBy:[]}], player:{score:5, drones:map[3:{id:3, x:3, y:3, emergency:3, battery:3, light:false} 4:{id:4, x:4, y:4, emergency:4, battery:4, light:false}]}, foe:{score:10, drones:map[5:{id:5, x:5, y:5, emergency:5, battery:5, light:false}]}, round:0}" != text2 {
...
func TestStateSetters(t *testing.T) {
	...
	if state1.Round != 0 {
		t.Error("State 1 round should be 0")
	}
	...
	if state2.Round != 1 {
		t.Error("State 2 round should be 1")
	}
	...
	if state3.Round != 1 {
		t.Error("State 3 round should be 1")
	}
}
